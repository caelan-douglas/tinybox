...
func set_footstep_sound() -> void:
	if standing_on_object is Brick:
		if standing_on_object._material == current_footstep_material:
			return
...
func entered_seat(path_to_seat : String) -> void:
	...
	if can_enter_seat and (_state == IDLE || _state == AIR || _state == RUN || _state == DIVE || _state == SLIDE || _state == SLIDE_BACK || _state == SWIMMING || _state == SWIMMING_IDLE):
		...
		UIHandler.show_alert("hint_stop_driving", 6)
		for motor : Node in seat_occupying.attached_motors:
			if motor is MotorFlyBrick:
				UIHandler.show_alert("hint_thruster", 6)
				break
		for motor : Node in seat_occupying.attached_motors:
			if motor is CannonBrick:
				if motor.automatic: continue
				
				UIHandler.show_alert("hint_shoot_cannon", 6)
				break
