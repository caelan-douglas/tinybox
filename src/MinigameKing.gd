...
var player_hill_time : Dictionary = {}
...
func king_winner(king_id : int, king_name : String) -> void:
...
func check_hill() -> void:
	...
	if multiplayer.is_server():
		...
		for p : RigidPlayer in players_in_hill:
...
func show_kings_score_on_playerlist(mode : bool) -> void:
	var player_list : Control = get_tree().current_scene.get_node("GameCanvas/PlayerList")
	for entry in player_list.get_children():
		var kings_score : Control = entry.get_node("HBoxContainer/KingsScore")
		kings_score.visible = mode

@rpc("any_peer", "call_local", "reliable")
func update_player_list(player_name : String, player_score : int) -> void:
	var player_list : Control = get_tree().current_scene.get_node("GameCanvas/PlayerList")
	for entry in player_list.get_children():
		if str(player_name) == str(entry.get_node("HBoxContainer/Label").text):
			var kings_score : Label = entry.get_node("HBoxContainer/KingsScore")
