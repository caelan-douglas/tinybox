...
func _init() -> void:
	...
	properties_to_save = ["global_position", "global_rotation", "brick_scale", "immovable", "indestructible", "magnetic_field",]
...
func _physics_process(delta : float) -> void:
	...
	for other_magnet in magnet_area_others:
		...
		north_pos = transform.basis.y * brick_scale.y/2 #up vector relative to brick * 
		south_pos = - north_pos
		
		var distance : float = (other_magnet.global_position - global_position).length()
		var force_magnitude : float = clampf(1 / distance, 0.001, 1)
		
		var northA_northB : Vector3 = 1000* force_magnitude * (other_magnet.north_pos - north_pos)
		var northA_southB : Vector3 = 1000* force_magnitude * (other_magnet.north_pos - south_pos)
		var southA_northB : Vector3 = 1000* force_magnitude * (other_magnet.south_pos - north_pos)
		var southA_southB : Vector3 = 1000* force_magnitude * (other_magnet.south_pos - south_pos)
