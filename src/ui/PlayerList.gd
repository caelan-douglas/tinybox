...
@onready var player_list_entry = preload("res://data/scene/ui/PlayerListEntry.tscn")
...
func update_list() -> void:
	for entry in get_children():
		for player in Global.get_world().rigidplayer_list:
			if str(player.name) == str(entry.name):
				var k = entry.get_node("HBoxContainer/K")
				var d = entry.get_node("HBoxContainer/D")
				var player_team = entry.get_node("HBoxContainer/Team")
				...
				if Global.get_world().get_current_map().get_teams().get_team(player.team) != null:
					var team_colour = Global.get_world().get_current_map().get_teams().get_team(player.team).colour
...
func sort_by_teams() -> void:
	...
	sorted_nodes.sort_custom(
		func(a: Node, b: Node): return a.get_node("HBoxContainer/Team").text < b.get_node("HBoxContainer/Team").text
	)
...
func add_player(player : RigidPlayer) -> void:
	for l in get_children():
		...
		if l.name == str(player.name):
			return
	# otherwise, continue
	var player_list_entry_i = player_list_entry.instantiate()
	var player_label = player_list_entry_i.get_node("HBoxContainer/Label")
	var player_team = player_list_entry_i.get_node("HBoxContainer/Team")
	var player_k = player_list_entry_i.get_node("HBoxContainer/K")
	var player_d = player_list_entry_i.get_node("HBoxContainer/D")
...
func remove_player_by_id(id : int) -> void:
	remove_player(Global.get_world().get_node_or_null(str(id)))
