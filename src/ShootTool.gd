...
@export var tool_name = "Shoot Tool"
...
@export var max_ammo_restore = 0
@export var fire_sound = false

@onready var restore_timer = Timer.new()
...
var shot_cooldown_counter = 0
var ui_progress = null

@onready var ball = preload("res://data/scene/clay_ball/ClayBall.tscn")
@onready var brick = preload("res://data/scene/brick/Brick.tscn")
@onready var rocket = preload("res://data/scene/rocket/Rocket.tscn")
@onready var bomb = preload("res://data/scene/bomb/Bomb.tscn")
@onready var water = preload("res://data/scene/water/WaterProjectile.tscn")
@onready var fire = preload("res://data/scene/fire/FireProjectile.tscn")
...
func show_floaty_cost_client(cost) -> void:
	# floaty cost text
	if tool_visual:
...
@rpc("any_peer", "call_local", "reliable")
func _set_tool_audio_playing(mode):
	match(mode):
		true:
			audio_anim.stop()
			audio.volume_db = -10
