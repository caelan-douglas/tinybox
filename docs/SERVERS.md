...
Community-run servers can be added to the in-game list!
...
- Any server that displays inappropriate, hurtful or abusive messages may be removed.
- Make sure you note the admins of the server in the `hosted_by` section.
- Servers that run a modified version of the code should have a different version number so that the official client is not able to join a potentially incompatible server.
